...
public class DissolverScreen extends AbstractAlchemistryScreen<DissolverScreenHandler> {
    ...
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(Alchemistry.MOD_ID, "textures/gui/dissolver_gui.png");

    protected final List<DisplayData> displayData = new ArrayList<>();

    public DissolverScreen(DissolverScreenHandler menu, Inventory inventory, Component title) {
        super(menu, inventory, title, 184, 200);
        displayData.add(new ProgressDisplayData(menu.getPropertyDelegate(), 0, 1, 88, 34, 60, 9, Direction2D.DOWN));
        displayData.add(new EnergyDisplayData(menu.getPropertyDelegate(), 2, 3, 156, 12, 16, 54));
    }
    ...
    protected void extractLabels(GuiGraphicsExtractor matrices, int mouseX, int mouseY) {
        MutableComponent title = Component.translatable("alchemistry.container.dissolver");
