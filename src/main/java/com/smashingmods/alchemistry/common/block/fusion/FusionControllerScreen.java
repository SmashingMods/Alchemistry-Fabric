...
public class FusionControllerScreen extends AbstractAlchemistryScreen<FusionControllerScreenHandler> {

    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(Alchemistry.MOD_ID, "textures/gui/fusion_gui.png");

    protected final List<DisplayData> displayData = new ArrayList<>();

    public FusionControllerScreen(FusionControllerScreenHandler menu, Inventory inventory, Component title) {
    ...
    protected void extractLabels(GuiGraphicsExtractor matrices, int mouseX, int mouseY) {
        MutableComponent title = Component.translatable("alchemistry.container.fusion_controller");
